...
class _AllprodukState extends State<Allproduk> {
  ...
  final listpaket = new List<Listpaket>();

  var loadingdokter = false;

  List<Listpaket> _list = [];
  List<Listpaket> _search = [];
  ...
  tampildata() async {
    ...
    final responsee = await http.get(Uri.parse(Restapi.paketgym));
    if (responsee.statusCode == 200) {
      final data = jsonDecode(responsee.body);
      print(data);
      setState(() {
        for (Map i in data) {
          _list.add(Listpaket.fromJson(i));
          loadingdokter = false;
        }
      });
    } else {
  ...
  _onsearch(String text) async {
    ...
    _list.forEach((f) {
      if (f.namapaket.contains(text) || f.namapaket.contains(text))
        _search.add(f);
    });
    setState(() {});
  }
    ...
    return MaterialApp(
        ...
        home: Scaffold(
            ...
            appBar: new AppBar(
                ...
                title: Text("All Membership Packages",
                    style: TextStyle(
                        fontSize: 20,
                        color: Colors.white,
                        fontFamily: 'Kali'))),
                      ...
                      Container(
                        ...
                        child: new TextField(
                          ...
                          decoration: InputDecoration(
                              ...
                              hintText: "Which membership will choose?",
